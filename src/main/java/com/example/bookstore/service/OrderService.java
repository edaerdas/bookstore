...
import com.example.bookstore.exception.QuantityNotConfirmed;
...
public class OrderService {
    ...
    public Long createOrder(OrderCreateDto orderCreateDto) throws EntityNotFound, QuantityNotConfirmed {
        ...
        for (OrderItemDto orderItemDto : orderCreateDto.orderItemDtoList) {
            if (orderItemDto.quantity <= 0)
                throw new QuantityNotConfirmed("Quantity should be greater than 0");
            ...
            if (book.getStockQuantity() < orderItemDto.quantity)
                throw new QuantityNotConfirmed("Book stock not enough for sale");
